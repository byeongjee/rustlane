...
fn emit(name: &Ident, vis: &Visibility, fields: &[FieldInfo]) -> TokenStream {
    ...
    let struct_fields = fields.iter().map(|f| {
        ...
        let ty = &f.ty;
        match f.kind {
            FieldKind::Varying => quote!(pub #id: <#ty as ::spmd::SpmdValue>::Varying<N>),
    ...
    let splat_fields = fields.iter().map(|f| {
        let id = &f.ident;
        match f.kind {
            FieldKind::Varying => quote!(#id: ::spmd::SpmdValue::splat(self.#id)),
    ...
    let assign_stmts: Vec<TokenStream> = fields
        ...
        .map(|f| {
            ...
            match f.kind {
                FieldKind::Varying => {
                    quote!(::spmd::MaskedAssign::masked_assign(&mut self.#id, __exec, value.#id);)
                }
    ...
    let masked_assign_impl = |ctx: TokenStream| {
        ...
        quote! {
            impl<const N: usize> ::spmd::MaskedAssign<#ctx> for #vname<N>
            where
                ::core::simd::LaneCount<N>: ::core::simd::SupportedLaneCount,
            {
                #[inline(always)]
    ...
    masked_assign.extend(masked_assign_impl(quote!(::spmd::AllOn)));
    masked_assign.extend(masked_assign_impl(quote!(::spmd::BoolGuard)));
    if all_varying {
        masked_assign.extend(masked_assign_impl(quote!(::spmd::VMask<N>)));
        masked_assign.extend(masked_assign_impl(quote!(::spmd::VMaskGuard<N>)));
    }
    ...
    let gather_and_select = if all_varying {
        let gather_fields = fields.iter().map(|f| {
            ...
            quote! {
                #id: unsafe {
                    <#ty as ::spmd::SpmdGather>::gather_fields::<__Base, N, __E>(
                        __base,
                        __idx,
        ...
        quote! {
            impl ::spmd::SpmdGather for #name {
                #[inline(always)]
                unsafe fn gather_fields<__Base, const N: usize, __E>(
                    __base: &[__Base],
                    __idx: ::spmd::Varying<i32, N>,
                    __field_offset: usize,
                    __exec: __E,
                ) -> #vname<N>
                where
                    __E: ::spmd::ActiveMask<N> + ::core::marker::Copy,
                    ::core::simd::LaneCount<N>: ::core::simd::SupportedLaneCount,
                {
                    #vname { #(#gather_fields,)* }
                }
            ...
            {
                #[inline(always)]
                pub fn gather<__E: ::spmd::ActiveMask<N> + ::core::marker::Copy>(
                    base: &[#name],
                    idx: ::spmd::Varying<i32, N>,
                    exec: __E,
                ) -> Self {
                    unsafe {
                        <#name as ::spmd::SpmdGather>::gather_fields::<#name, N, __E>(base, idx, 0, exec)
                    }
                }
