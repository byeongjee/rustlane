...
fn parse_targets(attr: TokenStream) -> Result<Vec<Target>, Error> {
    ...
    if !meta.path.is_ident("targets") {
        return Err(Error::new_spanned(
            &meta.path,
            "#[spmd::export] only accepts `targets(\"..\", ..)`",
        ));
    }
...
fn check_no_varying(ty: &Type, errors: &mut Vec<Error>) {
    ...
    if let Some(sp) = f.hit {
        errors.push(Error::new(
            sp,
            "`Varying` cannot appear in an #[spmd::export] signature: its in-memory \
...
fn check_no_generics(func: &ItemFn, errors: &mut Vec<Error>) {
    for gp in &func.sig.generics.params {
        errors.push(Error::new_spanned(
            gp,
            "#[spmd::export] fns take no generic parameters (use elided lifetimes on \
             `&[T]`/`&mut [T]` args)",
        ));
    }
}
...
pub fn expand(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let func = match parse2::<ItemFn>(item) {
        ...
        Err(_) => {
            return Error::new(
                Span::call_site(),
                "#[spmd::export] applies to free functions with an all-uniform signature",
            )
            .to_compile_error();
        }
    };
...
fn gen_dispatch(
    ...
    for t in &x86 {
        ...
        let n = t.n;
        let call = quote! { #impl_name::<#n, _>(::spmd::AllOn, #(#arg_names),*) };
    ...
    let (x86_support, x86_wrapper_body): (TokenStream, TokenStream) = if x86.is_empty() {
        (
            ...
            quote! {
                compile_error!(
                    "#[spmd::export]: no x86_64 target in `targets(..)`; add one (e.g. \"avx2\")"
                );
            },
        )
    } else {
    ...
    let arm_wrapper_body: TokenStream = match arm.first() {
        Some(t) => {
            let n = t.n;
            quote! { return #impl_name::<#n, _>(::spmd::AllOn, #(#arg_names),*); }
        }
        None => quote! {
            compile_error!("#[spmd::export]: no aarch64 target selected");
        },
    };
    ...
    quote! {
        ...
        #vis fn #name(#inputs) #output {
            ...
            {
                unimplemented!("#[spmd::export] supports x86_64 and aarch64 targets")
            }
        }
    }
}
