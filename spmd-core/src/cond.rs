...
#[diagnostic::on_unimplemented(
    message = "`{Self}` cannot be order-compared with `{Rhs}` in an spmd kernel",
    label = "no spmd ordering between these types",
...
#[diagnostic::on_unimplemented(
    message = "`{Self}` cannot be equality-compared with `{Rhs}` in an spmd kernel",
    label = "no spmd equality between these types",
    note = "supported: uniform scalar vs uniform scalar (-> `bool`), and any mix of \
            `Varying<T, N>` and its scalar `T` (-> `Mask<i32, N>`)"
)]
...
#[diagnostic::on_unimplemented(
    message = "`{Self}` cannot appear on the left of `&&` in an spmd kernel",
...
#[diagnostic::on_unimplemented(
    message = "`!` cannot be applied to `{Self}` in an spmd kernel condition",
    label = "expected `bool` or `Mask<i32, N>`"
)]
pub trait SpmdNot {
    type Out;
