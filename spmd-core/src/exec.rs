...
#[diagnostic::on_unimplemented(
    message = "`{Self}` is not an spmd execution context",
    label = "expected `AllOn`, `BoolGuard`, `VMask<N>` or `VMaskGuard<N>`",
    note = "`__exec` values are produced by the `#[spmd::kernel]` rewrite; \
...
#[diagnostic::on_unimplemented(
    message = "an spmd `if`/`while` condition must be `bool` (uniform) or `Mask<i32, N>` (varying), not `{C}`",
...
#[diagnostic::on_unimplemented(
    message = "cannot start an spmd `while` loop from context `{Self}` with a `{C}` condition",
...
#[diagnostic::on_unimplemented(
    message = "cannot start an spmd `for`/`loop` from context `{Self}`",
    label = "not an spmd execution context"
)]
...
#[diagnostic::on_unimplemented(
    message = "spmd loop state `{Self}` cannot be narrowed by a `{C}` condition",
...
#[diagnostic::on_unimplemented(
    message = "`{Self}` is not an spmd loop state",
