...
pub fn expand(attr: TokenStream, item: TokenStream) -> TokenStream {
    if !attr.is_empty() {
        return Error::new_spanned(&attr, "#[rustlane::kernel] takes no arguments")
            .to_compile_error();
    }
    match syn::parse2::<Item>(item) {
        ...
        Ok(other) => Error::new_spanned(
            &other,
            "#[rustlane::kernel] applies to free functions and inherent `impl` blocks",
        )
        .to_compile_error(),
...
fn expand_impl(imp: ItemImpl) -> TokenStream {
    ...
    if let Some((_, path, _)) = &imp.trait_ {
        return Error::new_spanned(
            path,
            "#[rustlane::kernel] impl blocks must be inherent impls (trait impls would change the \
            ...
            ImplItem::Fn(m) => {
                ...
                if defaultness.is_some() {
                    errors.push(Error::new_spanned(
                        &defaultness,
                        "`default` methods are not supported in #[rustlane::kernel] impls",
                    ));
                }
    ...
    quote! {
        {
            ...
            if <__E as ::rustlane::Exec>::UNIFORM {
                let __exec = ::rustlane::AllOn;
                #(#stmts)*
            } else {
                let __exec = ::rustlane::VMask::<N>(::rustlane::ActiveMask::<N>::active_mask(__exec));
                #(#stmts)*
            }
        }
    }
