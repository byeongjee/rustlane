...
impl Rewriter {
    ...
    fn emit_foreach(&mut self, mac: Macro, out: &mut Vec<Stmt>) {
        ...
        out.push(parse_quote_spanned!(sp=> {
            ...
            let __n: usize = #end2;
            while __base + N <= __n {
                let #var = ::rustlane::LinearIndex::<N>::new(__base);
                #body2
                __base += N;
            }
            if __base < __n {
                let __exec = __exec.and_cond(::rustlane::VMask::<N>::first(__n - __base).0);
                let #var = ::rustlane::LinearIndex::<N>::new(__base);
                #body2
            }
        }));
    ...
    fn emit_foreach_2d(&mut self, mac: Macro, out: &mut Vec<Stmt>) {
        ...
        out.push(parse_quote_spanned!(sp=> {
            ...
            while __y < __y1 {
                ...
                while __base + N <= __x1 {
                    let #xvar = ::rustlane::LinearIndex::<N>::new(__base);
                    #body2
                    __base += N;
                }
                if __base < __x1 {
                    let __exec = __exec.and_cond(::rustlane::VMask::<N>::first(__x1 - __base).0);
                    let #xvar = ::rustlane::LinearIndex::<N>::new(__base);
                    #body2
                }
                __y += 1;
    ...
    fn emit_foreach_tiled(&mut self, mac: Macro, out: &mut Vec<Stmt>) {
        ...
        out.push(parse_quote_spanned!(sp=> {
            ...
            let __dy: ::rustlane::Varying<i32, N> =
                ::rustlane::Varying::from_array(::core::array::from_fn(|__l| (__l / __tx) as i32));
            let __dx: ::rustlane::Varying<i32, N> =
                ::rustlane::Varying::from_array(::core::array::from_fn(|__l| (__l % __tx) as i32));
            let __y1: usize = #yend2;
            let __x0: usize = #xstart2;
            let __x1: usize = #xend2;
            ...
            while __by < __y1 {
                ...
                while __bx < __x1 {
                    let #yvar = ::rustlane::Varying::<i32, N>::splat(__by as i32) + __dy;
                    let #xvar = ::rustlane::Varying::<i32, N>::splat(__bx as i32) + __dx;
    ...
    fn rw(&mut self, e: Expr) -> Expr {
        match e {
            ...
            Expr::Cast(c) => {
                ...
                let ty = c.ty;
                parse_quote_spanned!(sp=> ::rustlane::SpmdCast::<#ty>::spmd_cast(#inner))
            }
            Expr::Closure(c) => {
                self.err(
                    &c,
